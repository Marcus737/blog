...
export default {
  ...
  methods: {
    async getDetail(id) {
      ...
      let c = data.content;
      data.textLen =c.replace(/!\[.*\]\(.*\)/g, "").length
      this.detail = data;
    },
    markdownRender() {
      marked.setOptions({
    ...
    async getComData () {
      ...
      const result = await list(data.articleId, data.pageNum, data.pageSize);
    ...
    async likeChange (e) {
      if (!e) { // 未喜欢这篇文章
        if (!localStorage.getItem('userInfo')){
          this.$message({ type: 'error', message: '未登录', offset: 60 })
          return;
        }
        const data = {
        ...
        const result = await setLike(data.articleId, data.userId)
