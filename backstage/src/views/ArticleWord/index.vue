...
<script>
import { articleWordlist, deleteArticleWord } from '@/api/word'
...
export default {
  ...
  methods: {
    async getLists () {
      this.loading = true
      const result = await articleWordlist(this.page.pageNum, this.page.pageSize)
    ...
    async handleDelete (i, id ) {
      const result = await deleteArticleWord(id.id)
      if (result) {
